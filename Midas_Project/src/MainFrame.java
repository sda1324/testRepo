...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// s.save(project);
				/*
				 * ArrayList<Furniture> a = new ArrayList<Furniture>(); for(Furniture f : a )
				 * f.RemoveFurniture(); ArrayList<Room> b = new ArrayList<Room>(); for(Room r :
				 * b ) r.RemoveRoom(); project = new Project();
				 * 
				 * panel_1.removeAll(); panel_1.updateUI();
				 */
			}
		});
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				BufferedImage image = new BufferedImage(panel_1.getWidth(), panel_1.getHeight(),
	...
	class MyPanel extends JPanel {
		...
		public void drawOutline() { // 가장자리 벽면 그리는 함수
			...
			s = new Rectangle2D.Float(mainRoom.north.getX(), mainRoom.north.getY(), mainRoom.north.getWidth(),
					mainRoom.north.getHeight());
			shapeArray.add(s);
			...
			s = new Rectangle2D.Float(mainRoom.east.getX(), mainRoom.east.getY(), mainRoom.east.getWidth(),
					mainRoom.east.getHeight());
			shapeArray.add(s);
			...
			s = new Rectangle2D.Float(mainRoom.south.getX(), mainRoom.south.getY(), mainRoom.south.getWidth(),
					mainRoom.south.getHeight());
			shapeArray.add(s);
			...
			s = new Rectangle2D.Float(mainRoom.west.getX(), mainRoom.west.getY(), mainRoom.west.getWidth(),
					mainRoom.west.getHeight());
			shapeArray.add(s);
		...
		void drawFurniture(MouseEvent e) {
			...
			case 'W': {
				...
				f.setX(e.getX());
				f.setY(e.getY());
				f.setURL("src/icon/washing_machine.png");
				f.setJPanel();
				// 겹치는 경우 처리
				if (containXY(e.getX(), e.getY())) {
					break;
			...
			case 'R': {
				...
				f.setY(e.getY());
				f.setJPanel();
				f.setURL("src/icon/refrigerator.png");
				// 겹치는 경우 처리
				if (containXY(e.getX(), e.getY())) {
					break;
				}
			 ...
			 * 
			 * Shape s; if(door.dir == 1) s = new
			 * Rectangle2D.Float(project.basic_x+door.first_x,
			 * project.basic_y+door.first_y-3, 30, 6); else s = new
			 * Rectangle2D.Float(project.basic_x+door.first_x-3,
			 * project.basic_y+door.first_y, 6, 30); shapeArray.add(s); repaint();
			 */
		}
	}
